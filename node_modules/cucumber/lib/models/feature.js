...
Feature =
function Feature(_ref) {var _this = this;var gherkinData = _ref.gherkinData,gherkinPickles = _ref.gherkinPickles,scenarioFilter = _ref.scenarioFilter,uri = _ref.uri;(0, _classCallCheck3.default)(this, Feature);
  ...
  var backgroundStepLines = _lodash2.default.chain(gherkinData.children).
  filter(['type', 'Background']).
  map('steps').
  flatten().
  map(function (step) {return step.location.line;}).
  value();
  ...
  this.scenarios = _lodash2.default.chain(gherkinPickles).
  map(function (gherkinPickle) {
  ...
  filter(function (scenario) {return scenarioFilter.matches(scenario);}).
  value();
